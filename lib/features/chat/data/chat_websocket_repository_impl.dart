...
import 'datasources/chat_websocket_remote_datasource.dart';

class ChatWebSocketRepositoryImpl implements ChatRepository {
  final ChatWebSocketRemoteDatasource datasource;

  ChatWebSocketRepositoryImpl({required this.datasource});

  @override
  Stream<List<ChatRoomEntity>> watchRooms() {
    return datasource.watchRooms();
  }

  @override
  Stream<List<ChatRoomStateEntity>> watchRoomStates() {
    return datasource.watchRoomStates();
  }

  @override
  Stream<List<ChatMessageEntity>> watchMessages(String roomId) {
    return datasource.watchMessages(roomId);
  }
  ...
  Future<List<ChatMessageEntity>> loadOlderMessages(
    String roomId, {
    DateTime? before,
    int limit = ChatWebSocketRemoteDatasource.messagesPageSize,
  }) {
  ...
  Stream<List<ChatPresenceEntity>> watchPresence(String roomId) {
    return datasource.watchPresence(roomId);
  }

  @override
  Stream<List<ChatTypingEntity>> watchTyping(String roomId) {
    return datasource.watchTyping(roomId);
  }
